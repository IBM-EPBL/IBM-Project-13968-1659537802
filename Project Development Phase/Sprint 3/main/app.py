...
# create_query="CREATE TABLE users(uname varchar(20) NOT NULL,DOB date NOT NULL,Mobile varchar(20) NOT NULL,email varchar(50) NOT NULL PRIMARY KEY,addr varchar(50) not null,city varchar(20),pincode varchar(20) NOT NULL,password varchar(20) NOT NULL,is_eligible varchar(20),joined TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);"
...
# ibm_db.exec_immediate(conn,drop_query)
# create_query="CREATE TABLE hospital(hospid varchar(20) NOT NULL PRIMARY KEY,hosp_name varchar(20),contact INTEGER,location varchar(50),pass varchar(20));"
# ibm_db.exec_immediate(conn,create_query)
# insert_quer="INSERT INTO hospital(hospid,hosp_name,contact,location,pass) VALUES ('TN195001123','Surya','123456789','CHENNAI','varun2000')"
...
def approved():
    ...
    if account:
        ...
        ibm_db.execute(stm)
        session["updated"]="You have donated blood and no longer eligible"
        return redirect('/dashboard')
    else:
...
@app.route('/donate')
def donate():
    sqll = "SELECT * FROM eligible,users WHERE eligible.elig_email =? AND users.is_eligible=?"
    stmtt = ibm_db.prepare(conn, sqll)
    ...
    if account:
        ...
        sql = "SELECT * FROM prequest,users WHERE prequest.blood_group =? AND prequest.location =? AND users.is_eligible =? AND prequest.priority =? AND prequest.blood_camp =? ORDER BY request_date ASC"
        stmt = ibm_db.prepare(conn, sql)
        ...
        while tuple != False:
            items.append(tuple)
            tuple = ibm_db.fetch_tuple(stmt)
        sqll = "SELECT * FROM prequest,users WHERE prequest.blood_group =? AND prequest.location =? AND users.is_eligible =? AND prequest.priority =? AND prequest.priority =? AND prequest.blood_camp =? ORDER BY request_date ASC"
        stmtt = ibm_db.prepare(conn, sqll)
        ibm_db.bind_param(stmtt, 1, session["bg"])
        ibm_db.bind_param(stmtt, 2, session["city"])
        ibm_db.bind_param(stmtt, 3, "True")
        ibm_db.bind_param(stmtt, 4, "LOW")
        ibm_db.bind_param(stmt, 5, "NO")
        ibm_db.execute(stmtt)
        item=[]
        tuple = ibm_db.fetch_tuple(stmt)
        while tuple != False:
            item.append(tuple)
            tuple = ibm_db.fetch_tuple(stmt)
        sq = "SELECT * FROM prequest,users WHERE prequest.location =? AND users.is_eligible =? AND blood_camp =? ORDER BY request_date ASC"
...
def hospitaldashboard():
    if not session.get("uid"):
        ...
        sql = "SELECT * FROM accepted,prequest WHERE accepted.hospital_id =? AND prequest.location =? ORDER BY prior ASC, donate_date ASC"
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt, 1, session['hospid'])
        ibm_db.bind_param(stmt, 2, session["hosplocate"])
        ibm_db.execute(stmt)
        item=[]
        tuple = ibm_db.fetch_tuple(stmt)
        while tuple != False:
